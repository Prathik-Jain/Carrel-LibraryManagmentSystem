...
-------------------------------------------------------------------------
 ...
 BEGIN 
	...
	INSERT INTO Book(ID,ISBN,TITLE,AUTHOR,PUBLISHER,EDITION,PRICE,RACK,LASTBORROWED)
	...
	WITH (
		...
		LASTBORROWED NVARCHAR(MAX) '$.LASTBORROWED'
	)
END
...
BEGIN
	...
	INSERT INTO Member (ID,FNAME,LNAME,PHONE,DEPT,SEM,MAXBOOKS,BORROWEDBOOKS,DUES)
	SELECT * FROM OPENJSON(@DATA)
	WITH (
		ID SMALLINT '$.ID',
		...
		DEPT NCHAR(3) '$.DEPT',
		SEM TINYINT '$.SEM',
		MAXBOOKS TINYINT '$.MAXBOOKS',
		BORROWEDBOOKS NVARCHAR(MAX) '$.BORROWEDBOOKS',
		DUES NVARCHAR(MAX) '$.DUES'
	)
END
